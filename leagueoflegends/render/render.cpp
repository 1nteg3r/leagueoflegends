	...
	namespace scriptsrender
	{
		...
		namespace debug
		{
			...
			void DrawAttackRange()
			{
				...
				if (!me->IsAlive())
					return;
				Vector2 screenPos = functions::WorldToScreen(me->GetPosition());
				if (!IsOnScreen(screenPos)) return;
				RenderCircleWorld(me->GetPosition(), 190, me->GetBoundingRadius() + me->GetAttackRange(), COLOR_GREEN, 3.0f);
			}
			...
			void DrawPlayerPaths()
			{
				...
				{
					auto obj = globals::heroManager->GetIndex(i);
					if (obj->IsAlive() && obj->IsVisible() && obj->IsEnemy())
					{
